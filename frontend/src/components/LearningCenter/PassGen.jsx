...
const PassGen = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen to-blue-800 p-4">
            <div className="bg-white p-6 rounded shadow-md text-center " style={{ width: '100%', maxWidth: '500px' }}>
                <div className="relative mb-4">
                    <input type={showPassword ? "text" : "password"} value={password} readOnly className="text-center w-full bg-gray-200 text-lg rounded p-2 pr-16" />
                    <button onClick={toggleShowPassword} className="absolute right-14 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-black">
                        {showPassword ? <FaEyeSlash size={20} /> : <FaEye size={20} />}
                    </button>
                    <button onClick={copyToClipboard} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-black pr-2">
                        <FaCopy />
                    </button>
                </div>
                <div className="mb-4">
                    <label htmlFor="length" className="block mb-2">Character Length: {length}</label>
                    <input type="range" id="length" min="12" max="20" value={length} onChange={(e) => setLength(e.target.value)} className="w-full" />
                </div>
